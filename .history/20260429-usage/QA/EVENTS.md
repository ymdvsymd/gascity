...
| **event bus** | システム内で起きた事実 (2026-05-13 時点で 46 種類) の追記専用ログ。**pull (`List`) と watch (`Watch().Next()`) の併用**で消費される。event そのものは agent 起動の直接 trigger ではなく、controller が拾って reconciler を起動する間接経路。HTTP `gc events` 経由でも `limit` が end-to-end で配線されているので、100MB クラスのログでも tail が ms オーダーで返る (§5.5, PR #1995) |
...
### 5.1 KnownEventTypes は 46 種、カテゴリで整理

`internal/events/events.go:79-102` に定数で定義された全 event 種別を、カテゴリ別に整理 (2026-05-13 時点):

| カテゴリ | 主な event | 主な発行者 |
|---|---|---|
| **Session lifecycle** | `session.woke` `session.stopped` `session.crashed` `session.draining` `session.undrained` `session.idle_killed` `session.max_age_killed` `session.suspended` `session.quarantined` `session.updated` | supervisor |
| **Bead operations** | `bead.created` `bead.closed` `bead.updated` | controller |
| **Mail** | `mail.sent` `mail.read` `mail.archived` `mail.marked_read` `mail.marked_unread` `mail.replied` `mail.deleted` | API / hook |
...
| **City lifecycle** | `city.created` `city.suspended` `city.resumed` `city.unregister_requested` | supervisor |
| **Request result** | `request_result.city_create` `request_result.city_unregister` `request_result.session_create` `request_result.session_message` `request_result.session_submit` `request.failed` | API (async ack) |
| **Provider / Worker** | `provider.swapped` `worker.operation` | controller / worker |
| **External messaging** | `extmsg.bound` `extmsg.unbound` `extmsg.group_created` `extmsg.adapter_added` `extmsg.adapter_removed` `extmsg.inbound` `extmsg.outbound` | API |

> **enforced by CI**: AGENTS.md の「Typed events」不変条件により、`KnownEventTypes` に並ぶ全 type に対して `RegisterPayload(constant, sample)` の登録が必須。`internal/events/payload.go:45-51` の `RegisterPayload` に対応 sample が無いと `TestEveryKnownEventTypeHasRegisteredPayload` で fail する。新しい event type を生やしたら同時に payload も登録する流れ。
...
### 5.5 HTTP 経由の `limit` 配線とパフォーマンス特性（2026-05-13 / #1995）

`gc events` は CLI が HTTP API を叩く構造で、city 用 endpoint は `GET /v0/city/{name}/events` です。2026-04-30 時点の handler はリクエストの `limit` パラメータを `events.Filter` に伝えておらず、内部では `ep.List(filter)` で **events.jsonl 全体を毎回パース** → in-memory slice を要求ページサイズに truncate という挙動でした。

これが PR #1995 で `optimizedTail` パターン（supervisor 側の handler が既に持っていた fast path）に揃いました（`internal/api/huma_handlers_events.go:53-` の `TailProvider` 分岐）。

性能の実測値（PR description より、100MB / 127,682 行のログ）:

| 呼び出し | 返す件数 | 所要時間 |
|---|---|---|
| `ReadFiltered{no limit}` | 127,682 | 3.906 s |
| `ReadFiltered{limit:1}` | 1 | 1.25 ms |
| `ReadFilteredTail{limit:100}` | 100 | 3.34 ms |
| `ReadLatestSeq` | header のみ | 1.22 ms |

実装の要点:

- **cursor なしの fast path**: provider が `events.TailProvider` を実装しているなら `tp.ListTail(filter, limit)` に降りる。filter 空のときは `LatestSeq()` を `Total` として採用する（append-only かつ gap-free なので `seq == count`）。
- **cursor 指定 or `TailProvider` 未対応の slow path**: 旧来通り `ep.List(filter)` を呼ぶが、`offset + limit` でスキャンを打ち切る。完全に full-scan には戻らない。
- **ソート順**: newest-first を保つ（supervisor handler と一致）。
- **`Total` の意味**: filter 空なら全件数（=`LatestSeq`）、filter ありなら best-effort で「返した件数」と同値（supervisor handler の precedent）。

運用上の影響:

- 100MB クラスの event log がある city で `gc events --seq` が 10s 超でタイムアウトしていた現象（#1987）が解消。
- dashboard の `GET /v0/events/stream` 初期ロードが速くなる（SSE で繋ぐ前の tail プレロードに同じ handler を使うため）。
- `events.jsonl` を `bd backup` 的に定期 truncate していた運用は、本修正後はそこまで必要ない。ただし無制限に膨張させると filesystem 側の inode やバックアップ容量に響くので、cron-based な rotate は引き続き推奨。

> **設計の流れ**: `events.Provider` は基本 interface で `List` / `LatestSeq` / `Watch` / `Close` の 4 つだけ。`TailProvider` は **opt-in な extension**（`events.go:143-147`）。FileRecorder（本番の jsonl backend）は実装しているが、テスト用の小規模 provider は実装しなくてもよく、自然と slow path へ降りる。
...
| `internal/events/events.go:79-102` | KnownEventTypes (2026-05-13 時点 46 種) |
| `internal/events/events.go:143-147` | `TailProvider` extension interface (#1995) |
| `internal/events/payload.go:39-51` | `RegisterPayload` / `KnownEventTypes` の payload 登録 |
| `internal/api/huma_handlers_events.go:53-` | city events handler の `optimizedTail` 分岐 (#1995) |
