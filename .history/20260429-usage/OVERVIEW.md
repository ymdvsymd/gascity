...
対象ユーザーは、複数の AI コーディングエージェントを「短時間しか使わない」のではなく「常時走らせて、メールと bead（タスク）で協調させる」運用に踏み込みたい開発者と、再利用可能なオーケストレーションパックを設計したい SDK 構築者である。Gas Town を使ってきた開発者にとっては、既存の役割を pack 化して持ち運ぶための受け皿となる。
...
| フォーミュラ | TOML で書く宣言的マルチステップワークフロー、`needs` で依存関係 | [COMMANDS.md#フォーミュラ](./COMMANDS.md#フォーミュラ) |
