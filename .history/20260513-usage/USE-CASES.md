...
- プロンプトテンプレートを手で書く代わりに、`gc prompt synth --role reviewer --provider codex --write` で LLM に雛形を書かせる手もある。生成後に手直しすれば良いので、初稿の手間を省ける。
- provider ごとに差し分けたい注意書きがあるなら、テンプレ内で `{{ templateFirst . (printf "slash-commands-%s" .ProviderKey) "slash-commands-fallback" }}` のように書いて、`template-fragments/slash-commands-claude.md` などを置けば、provider 別の fragment を最初に見つかったところで採用してくれる。
...
- 同じ formula を rig ごとに違う値で流したい場合は、city.toml の `[[rigs]]` に `formula_vars = { target_branch = "develop" }` のように書いておくと、その rig からの実行では `--var` を省略しても rig の値が効く。precedence は `--var` > `[[rigs]] formula_vars` > formula 側 `default` の順なので、CLI から個別に上書きすることもできる。詳細は [QA/FORMULA-VARS.md](./QA/FORMULA-VARS.md) を参照。
...
- pack 同梱の formula が公開している変数を rig ごとに違う既定値で流したいときは、`[[orders.overrides]]` で interval を書き換えるのと並べて、`[[rigs]] formula_vars = { target_branch = "develop" }` のように rig 側で値を渡せる。pack 側を触らず、`--var` も毎回打たずに済む。
...
- 走っているエージェントを止めずに設定だけ反映したい（例: `.gc/settings.json` の小さな変更）場合は handoff ではなく `gc reload --soft` を使う。config drift があっても session を drain しないので、エージェントが作業を続けたまま controller が新しい設定を取り込む。
