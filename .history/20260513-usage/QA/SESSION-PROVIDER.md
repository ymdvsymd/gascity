...
- **session provider** (§1) = controller が AI CLI プロセスとやり取りする手段 (tmux ペイン / 子プロセスの stdin / JSON-RPC / Kubernetes API など) を、`Start` / `SendKeys` / `Nudge` / `Peek` / `Stop` の 5 操作に揃えて、controller から手段の違いが見えないようにした層。
...
**controller が AI CLI と話しかけるやり方は色々あるけど、controller からは全部同じ操作に見えるように包んだもの**。

実際の「話しかけ方」は環境で違います:

- ローカル開発機なら → **tmux ペインにキーを送る**
- TTY を持たないバッチ実行なら → **子プロセスの stdin に書き込む**
- Claude Code のような構造化対話なら → **JSON-RPC でメッセージを送る** (ACP)
- Kubernetes 上なら → **Pod を立てて中の tmux に流す**

この違いを controller に見せない壁が session provider です。本書ではこの「話しかけ方 / やり取りの経路」を **transport (通信手段)** と呼びます。
...
| **session provider** (実行基盤) | `[session] provider = "tmux"` | そのバイナリをどこで・どう動かすかの通信手段 (transport) | `tmux` / `subprocess` / `acp` / `k8s` |
...
本書では §1 で **session provider** を「controller が AI CLI プロセスに話しかける手段を共通の 5 操作にまとめた層」と定義しました。これと対称的に、本章では **gc プロセス自身のライフサイクル**を抽象化する層 — 本書で「**外部 supervisor**」と呼ぶ — を整理します。systemd と Kubernetes はこの抽象の異なる具象実装です。
...
|---|---|---|---|
| session provider | controller ⇄ AI CLI | 通信手段 (Start / SendKeys / Nudge / Peek / Stop) | §1, §3 |
...
- **session provider** (§1) = controller と AI CLI の通信手段 (tmux / 子プロセス / JSON-RPC / Kubernetes など) を 5 操作に揃えた層。`internal/runtime/` 配下に 7 種の実装。
