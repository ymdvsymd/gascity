...
> 関連ドキュメント: `.history/20260429-usage/OVERVIEW.md` §5、`docs/tutorials/04-communication.md`、`.history/20260429-usage/QA/PROVIDER-ABSTRACTION.md` §2.7（hook 接続の wired / unwired 分類）
...
| `PreCompact` | `gc handoff --auto "context cycle"` | コンテキスト圧縮前の状態保全（claude / codex / cursor / gemini / **copilot**［2026-05-13 追加 #2009］） | × |
| `Stop` | `gc hook --inject` | 終了時の最終チェック | × |

> **`awaySummaryEnabled: false`（2026-05-13 / #2043）**: Claude Code の default settings に「席を外している間の要約」を生成しない設定が入った。理由は、chain agent（自律実行する claude 同士が連鎖する構成）で away summary 生成が stall の引き金になるケースがあり、それを止めるため。messaging の挙動そのものは変わらないが、長時間 attach せず放置している人間が「あとで away summary を確認する」運用は default では効かない点だけ留意。
...
サポート provider は `["claude", "codex", "gemini", "kiro", "opencode", "copilot", "cursor", "pi", "omp"]` (`internal/hooks/hooks.go:30`、2026-05-13 時点 9 種)。**provider 抽象が引き受けているのは「どう書き出すか」だけ**で、「何を / いつ / 誰に届けるか」は全部 bead クエリの話に落ちています。

**unwired な provider（amp / auggie）**: `internal/hooks/hooks.go:54` の `unwiredHookProviders = []string{"amp", "auggie"}` に分類されており、CLI 側に hook 機構はあるが gc 側で wire していない（QA/PROVIDER-ABSTRACTION.md §2.7 参照）。「hook が無いから mail が届かない」と誤解されがちだが、**nudge / mail のキュー drain は `worker.Handle` 経由で wire 不要で動く**ので、unwired provider にも mail は届きます。違いが出るのは「PreCompact のタイミングで `gc handoff` を発火させる」など、provider 由来の event に lock-step で動かしたい類の機能だけです。
